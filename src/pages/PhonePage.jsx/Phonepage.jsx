...
const PhonePage = () => {
  // Import des Produits et configuration via le contexte
  const { loading, products, setLimit, currentPage, setCurrentPage, totalPages, setCategoryId } = useProduct();
  ...
  const filteredProducts = products.filter(
    (product) =>
      selectedMark.includes("All") ||
      selectedMark.includes(product.mark.toLowerCase())
  );
  ...
  return loading ? (
    ...
    <div className="w-[90%] mx-auto min-h-screen bg-transparent dark:text-white py-[100px]">
            ...
            <ul className="space-y-2">
              {marks.map((mark) => (
                <li key={mark} className="flex items-center">
                  ...
                  />
                  <label htmlFor={mark} className="text-gray-700 dark:bg-slate-800 dark:text-white">
                    {mark}
                  </label>
                </li>
              ))}
